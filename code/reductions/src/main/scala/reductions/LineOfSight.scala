...
object LineOfSight {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    while(c < until){
      maxAngle = max(maxAngle, input(c) / c)
      c = c + 1
    }
    maxAngle
  }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if(input.length <= threshold)
      ...
      val middle = (end - from) / 2 + from
      new Node(upsweep(input, from, middle, threshold), upsweep(input, middle, end, threshold))
    }
  }
